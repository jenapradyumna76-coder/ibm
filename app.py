...
# Crucial: Fixes FileNotFoundError by ensuring the directory exists on the server
...
if uploaded_file:
    ...
    if st.button("🚨 PERFORM FULL FORENSIC SWEEP"):
        ...
        with st.status("Performing Comprehensive Scan...", expanded=True) as status:
            ...
            if ret:
                ...
                ax.set_title("Detection Probability (Scan Over 5 Frames)")
        ...
        pdf.cell(0, 7, f"SHA-256 HASH: {v_hash}", 0, 1)
        pdf.cell(0, 7, f"OFFICER: {investigator}", 0, 1)

        pdf.chapter_header("2. TEMPORAL ANOMALY SCAN")
        pdf.image(chart_path, w=150)
        
        # NEW SECTION: Heatmapping in PDF
        pdf.chapter_header("3. VISUAL ARTIFACT LOCALIZATION (HEATMAP)")
        pdf.image(grad_path, w=100)
        pdf.set_font("Arial", 'I', 9)
        pdf.multi_cell(0, 7, "Heatmap Interpretation: Red zones indicate areas where the AI identified neural texture inconsistencies common in synthesized media.")

        pdf.chapter_header("4. EXECUTIVE DETERMINATION")
        verdict = "TAMPERED / DEEPFAKE" if score > 0.5 else "AUTHENTIC CONTENT"
        pdf.set_font("Arial", 'B', 14)
        pdf.cell(0, 10, f"VERDICT: {verdict}", 0, 1)
        pdf.set_font("Arial", '', 10)
        pdf.multi_cell(0, 7, f"Average Confidence: {score*100:.2f}%. Notes: {notes}")

        pdf_path = "forensic_results/Final_Forensic_Report.pdf"
        pdf.output(pdf_path)
        
        with open(pdf_path, "rb") as f:
            st.download_button("📥 Download Official Forensic Certificate", f, file_name="Forensic_Report.pdf")

        st.image(grad_path, caption="Visual Forensic Evidence (Heatmap)")
